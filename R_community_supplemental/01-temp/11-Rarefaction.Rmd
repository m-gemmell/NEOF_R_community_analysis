...
Biologically, this makes sense as there is not bias being introduced by media.

## Rarefy data

The minimum sample depth appears to be a good choice for rarefying.
It keeps all the samples and at this depth the samples have a very good coverage of the ASVs present in the data. In your own analyses you may need to balance the loss of samples with the loss of depth.
...
num_asvs_vec["rarefied"] <- nrow(phyloseq::otu_table(pseq_relabund))
num_asvs_vec
```

Once you have viewed the outputs save your `phyloseq` object and ASC count vector.

```{r, eval=FALSE}
#Phyloseq save
save(pseq_relpseq_rarefyabund, file ="phyloseq_rarefied.RData")
#ASV count save
save(num_asvs_vec, file="num_asvs_vec.v3.RData")
```
