...
Unfortunately many of the R packages for community analysis do not contain inbuilt rarefaction method with their diversity methods.
...
Building up to multiple __iterations__ of __rarefaction__ we will first carry out one round of __rarefaction__ on our environmental phyla relative abundance data.
...
__Note:__ This is for demonstrtaive purposes and not something you would do in your real analysis.
...
#Process our long data frame
#Subset our long alpha diversity data frame to only contain our metrics of choice
metrics <- c("diversity_inverse_simpson", "evenness_pielou","diversity_fisher")
subset_alpha_metrics_long_df <- meta_alpha_mean_long_df[
  meta_alpha_mean_long_df$metric %in% metrics,
]
#Change instances of diversity_inverse_simpson to invsimpson
subset_alpha_metrics_long_df$metric <- gsub(pattern = "diversity_inverse_simpson",
                                           replacement = "invsimpson",
...
subset_alpha_metrics_long_df$metric <- factor(x = subset_alpha_metrics_long_df$metric,
                                             levels = c("pielou","invsimpson","fisher"))
#Check level order of metric factor column
...
To ensure our output works with subsequent ordination step we'll convert the output of `rbiom::unifrac()` into a matrix with `as.matrix()`.
...
We do not want this but thankfully we can the function `t()` to our abundance object to transpose the data.
...
To determine if their is a significant overall difference between the media & site groups we can carry out a PERMANOVA test.
...
if (identicial(row.names(beta_df_mean_noenv), row.names(pcoa_points)) == FALSE) {
  beta_df_mean_noenv <- beta_df_mean_noenv[row.names(pcoa_points),]
}
#Make points with metadata data frame
...
#Extract metadata and ensure row names order matches
metadf <- phyloseq::sample_data(pseq)
if (identicial(row.names(metadf), row.names(nmds_res$points)) == FALSE) {
