...
The sessions will start with a brief presentation followed by self-paced computer practicals guided by an online interactive book. The book will contain theory and practice code. This will be reinforced with multiple choice questions that will recap concepts and aid in interpretation of results.
...
Each sample and media combination was produced in replicates of three giving a total of 27 samples (3 X 4 X 3 = 36). The three replicates were cultured on three different plates with the same media. An ASV table, taxonomy table, and phylogenetic tree were produced with QIIME2 and DADA2.
...
We have a saved `ggplot2` object but now wee need to save and display it.
...
You can copy and paste the code changing all hte instances of __site__ to __media__.
...
rec_ancom_class <- ancom_class_out£res
#Remove large object
rm(ancom_class_out)
...
Great, now we can the biomarker with higher abundances in the ENV sample (blue) and higher abundances in the comparison media (red).
...
It seems that CVP and KBC are more simialr to each that to TSA.
...
pseq_relabund <- microbiome::transform(pseq, "compositional")
#Class phyloseq
class_relabund_pseq <- microbiome::aggregate_taxa(class_relabund_pseq, "Class", verbose = FALSE)
#Produce heatmap ggplot
...
Interestingly the CVP and KBC samples had relatively tight cluster that were quite near each other.
...
After diversity analysis we carried out differential abundance analysis in [chpater 17](#DA_chap).
...
You could try to remove all bu the ENV samples to determine this. 
...
Below are various resources if youwould like to learn more about phyloseq and associated packages.
