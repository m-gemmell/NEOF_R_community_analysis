...
<center>![](figures/NEOF_rounded_corners.png){style="width:300px"}</center>

# (PART\*) Intro {.unnumbered}

# Introduction

<center>![](figures/R_community.png){style="width:200px"}</center>
...
<center>![](figures/data.png){style="width:200px"}</center>

## Dataset

<center>![](figures/freshwater_france.png){style="border-radius: 3px; width: 500px"}</center>
...
<center>![](figures/river.png){style="width:200px"}</center>
...
<center>![](figures/petri.png){style="width:200px"}</center>
...
<center>![](figures/sum_red.png){style="width:200px"}</center>
...
<center>![](figures/workflow.png){style="width:200px; border-radius:15px; background:white"}</center>
...
7.  [Rarefaction](#rarefaction_chap): Carry out sample depth normalisation with rarefactions
    -   This will be used for alpha and beta diversity analysis
...
- We can easily tell which R package each function comes from
  - This is useful for your future coding where you may copy some, but not all, commands from one script to another 
    - You will therefore know which packages you will need to load
...
<center>
![](figures/nsc200_logon.png){style="width:800px; border-radius:2px"}
</center>
...
<center>
![](figures/terminal_window.png){style="width:700px; border-radius:2px"}
</center>
...
<center>![](figures/jupyter_logo.png){style="border-radius: 15px; width: 200px; background-color: white"}</center>
...
-   It is lightweight
-   Other heavier weight programs, such as RStudio, would struggle in our HPC due to the graphical and CPU load
...
**Note:** There is a [video tutorial](#jup_vid_tut) of this chapter at the bottom of this page if you prefer to watch it.
...
<center>![](figures/jupyter_notebook_example_1.png){style="width: 1000px; border: solid #7490b5 2px; border-radius:2px"}</center>
...
<center>![](figures/jupyter_notebook_example_2.png){style="width: 1000px; border: solid #7490b5 2px; border-radius:2px"}</center>
...
<center>![](figures/jupyter_notebook_example_3.png){style="width: 1000px; border-radius: 15px"}</center>
...
```
) will will add a **cell** below your currently selected **cell**
...
<center>![](figures/jupyter_notebook_example_4.png){style="width: 1000px; border-radius: 15px"}</center>
...
<center>![](figures/r_save.png){style="width:100px"}</center>
...
<center>![](figures/jupyter_notebook_example_5.png){style="width: 1000px; border-radius: 15px"}</center>
...
`r hide("Markdown")` You won't need to know more about `Markdown` but if you are interested please see the [`Markdown` guide](https://www.markdownguide.org/basic-syntax/). `r unhide()`
...
<center>![](figures/jupyter_notebook_refresh.png){style="width: 1000px; border-radius: 15px"}</center>
...
<center>
![](figures/half_tablespoon.png){style="width:200px; background: white; border-radius:15px"}
</center>
...
- __The biodiversity of your samples__
...
- __The biomass of your samples__
...
- __Read depth of sample groups__
...
- `ggplot()`: This creates a `ggplot2` object, storing the information and aesthetics
  ...
  - The second option is the aesthetics (`aes()`) to plot
    - In this case we want the depth column to be plotted on the y-axis (`y=depth`) and the site column to be plotted on the x-axis (`x=site`)
- `+`: We need to have a `+` at the end of the `ggplot()` function to add the next component of the plot
- `geom_boxplot()`: This adds a __layer__ to our `ggplot2` object 
  - THe function converts the `ggplot` object, which is just information, into a boxplot
...
```{r, eval=FALSE}
#Summary of row means (genus) from the genus otu table
...
We will add the following options to `vegan::rarecurve()`:
  ...
  -   `sample`: Draws a vertical line at the specified depth
        Additionally, draws a horizontal line for each sample showing how many ASVs were discovered at the sampling depth
...
- __`replace`__: Whether to rarefy with replacement (`TRUE`) or without replacement (`FALSE`)
  - The default is `TRUE` (with replacement)
  - We recommend rarefying without replacement (`FALSE`)
...
With this we can see that all our samples have the correct depth (10,433) and very few ASVs have been lost (2551-2541 = 10).
I would definitely be happy with this outcome.
...
- __Shannon__: A measure of diversity where a higher number means higher diversity
  - Shannon's index accounts for both abundance and evenness of the features present
...
To convert our dot plot into a box plot we can add the layer `geom_boxplot()`.
...
The only significant differences are between ENV & KBC, ENV & CVP, and between KBC & TSA. 
...
In our plot axis 1 has a very high percentage variation explained value of 72.4%.
...
On the other hand, axis.2 (15.9%) seems to be primarily showing the difference between the TSA samples and the other samples, and within the TSA samples.
...
We can see that significant differences are found within the groups for media (P = 0.001) and site (P = 0.008).
...
We can see that there is a significant difference between the CVP and ENV samples (P = 0.001).
...
The biggest difference is that our 1st axis explains 95% of variation, a very large amount.
...
- Use different ordination methods (`?phyloseq:ordinate`)
  - Different methods will produce different __percentage of variation explained__ values
    - If your plot has low values try another method to see if you can get higher values
  - Many people will try out different methods and use the one with the highest __percentage of variation explained__ values
    - This is perfectly acceptable and encouraged by many
  - Others may have other thoughts about trying multiple different metrics and the possibility of p-hacking
    - Please see the following paper for more info: [The Power of Microbiome Studies: Some Considerations on Which Alpha and Beta Metrics to Use and How to Report Results](https://www.frontiersin.org/journals/microbiology/articles/10.3389/fmicb.2021.796025/full)
...
<center>
![](figures/parameter_blue.png){style="width:150px"}
</center>
...
- `fix_formula =`: The groupings within your data
  - It is important to add all groupings that may have an influence on the microbial abundances.
...
<center>
![](figures/output_file_blue.png){style="width:150px"}
</center>
...
<center>
![](figures/portrait.png){style="width:150px; border-radius:15px; background:white"}
</center>
...
conda install bioconductor-microbiome=1.28.0 bioconda::bioconductor-ancombc=2.8.0 \
bioconda::biom-format=2.1.7 bioconda::kraken-biom=1.2.0 conda-forge::r-tidyverse=2.0.0 \
conda-forge::r-devtools=2.5.2 conda-forge::r-ggforce=0.5.0 conda-forge::pandoc=3.6.1 \
conda-forge::r-plotly=4.12.0 conda-forge::r-stringi=1.8.7 conda-forge::r-irkernel=1.3.2 \
conda-forge::r-htmltools=0.5.9 conda-forge::r-ggpubr=0.6.3 conda-forge::r-rbiom=3.1.0 \
conda-forge::r-ape=5.8_1 conda-forge::r-xml2=1.4.0 conda-forge::r-dt=0.34.0 anaconda::pathlib=1.0.1 \
conda-forge::r-ggupset=0.4.1 conda-forge::r-patchwork=1.3.2
```

__Note:__ It is common that some tools/packages will conflict with others. To overcome this we have set versions by adding `=xx` after a tool name in `conda` (e.g. `r-htmltools=0.5.9`).
...
- [Phyloseq](https://joey711.github.io/phyloseq/): The original `phyloseq`
  - The R object is still great but the included tutorials and some functions are a bit outdated
- [microbiome](https://microbiome.github.io/tutorials/): A supplemental package to `phyloseq`
  - It contains some much needed functions
  - The tutorials are much clearer than the `phyloseq` tutorials but still not always the clearest
...
- __Shotgun metagenomics data__
  ...
  - The [biom format](https://github.com/biocore/biom-format) stands for the Biological Observation Matrix (BIOM) format
    - It is used and created by a variety of tools
...
- f1 = Frequency of <u id='singletons'>singletons</u> 
- f2 = Frequency of <u id='doubletons'>doubletons</u> 

__Note:__ This cannot be used with DADA2 derived ASVs. DADA2 removes <u id='singletons'>singletons</u> which Chao uses within its calculation.
...
- Weighted UniFrac: Takes into account feature presence/absence & abundance
  - I.e. the value is weighted by the abundances
