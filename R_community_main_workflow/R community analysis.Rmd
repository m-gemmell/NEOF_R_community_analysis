...
Unfortunately the `ancombc-2` analysis requires a different `mamba/conda` environmental. Before activateing it you will need to close your `jupyter-notebook` instance. To carry this out:

- Ensure you have have saved, and the close and halted all your open notebooks.
- In the `juptyer-notebook` explorer (where you can view al the files in the directory) press the __"Quit"__ button on the top right.
- Close `firefox`.

To run `jupyter-notebook` with the required `mamba` environment:

- Open a new bash terminal, right click on the light grey background and choose _-"Terminal"__
- In the new terminal run the following command: `. useancombc2025`
- Change directory to your workshop directory: `cd ~/R_community_workshop`
- Activate `jupyter-notebook`: `jupyter-notebook`
...
**Note:** The next 2 code sections use the packages `dplyr` & `tidyr` along with piping `%>%`. This is used as it would be very difficult to do these steps in base R and they are the code provided in the [`ANCOMBC2` tutorial](https://bioconductor.org/packages/release/bioc/vignettes/ANCOMBC/inst/doc/ANCOMBC2.html). If you would like to learn more about these packages please see the [R for Data Science book](https://r4ds.had.co.nz/).
...
bioconductor-microbiome bioconductor-ancombc=2.0.1 biom-format \
r-tidyverse r-devtools r-ggforce pandoc r-plotly r-irkernel r-htmltools=0.5.4
```

__Note:__ It is common that some tools/packages will conflict with others. To overcome this we can set the version by adding `=xx` after a tool name in `conda` (e.g. `r-htmltools=0.5.4`). We have set the version for `bioconductor-ancombc` as newer versions are designed for a different package (`mia`) than `phyloseq`. Therefore newer version of ancombc2 do not work in our workflow.
...
Specific versions of bioconductor pacages can be installed like below:
