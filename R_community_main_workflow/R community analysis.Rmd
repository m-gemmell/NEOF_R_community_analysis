...
# Rarefaction {#rarefaction_chap}
<center>
![](figures/whale.png){style="width:200px; background:white; border-radius:15px"}
</center>
...
We can crate an ordination plot with ellipses, circular shapes showing clustering of groups.
...
There is a lot of information in `ancom_class_out`. Therefore, lets extract the results we want to look at and then remove the large object.
...
Because we had comparisons with more that 1 grouping (media & site) we might have rows with no significant results. We will therefore remove these rows.
