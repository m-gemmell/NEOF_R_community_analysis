...
column_1 <- c(
 ...
 #13 Summary
   '[**Summary**](#sumchap)', 
 '[![](figures/recap.png){height="150"}](#sumchap)'
)
column_2 <- c(
  #2: R packages
 ...
 #4 Jupyter
   '[**Jupyter**](#jupyterchap)', 
 '[![](figures/jupyter_logo.png){height="150"}](#jupyterchap)',
 #6 Min reads
 ...
 '[![](figures/mamba_logo.png){height="150"}](#mambainstalls)'
)
...
## Dataset
<center>![](figures/freshwater_france.png){style="border-radius: 5px; width: 500px; border: 5px solid #333333"}</center>
...
<center>![](figures/phyloseq_input.png){style="border-radius: 15px; width: 800px; border: 5px solid #333333"}</center>
...
## Mamba
<center>![](figures/mamba_logo.png){style="border-radius: 15px; width: 200px; border: 5px solid #333333"}</center>
...
<center>![](figures/jupyter_logo.png){style="border-radius: 15px; width: 200px; border: 5px solid #333333; background-color: white"}</center>
...
<center>![](figures/jupyter_notebook_example_1.png){style="width: 1000px; border-radius: 15px; border: 5px solid #333333"}</center>
...
<center>![](figures/jupyter_notebook_example_2.png){style="width: 1000px; border-radius: 15px; border: 5px solid #333333"}</center>
...
<center>![](figures/jupyter_notebook_example_3.png){style="width: 1000px; border-radius: 15px; border: 5px solid #333333"}</center>
...
<center>![](figures/jupyter_notebook_example_4.png){style="width: 1000px; border-radius: 15px; border: 5px solid #333333"}</center>
...
<center>![](figures/jupyter_notebook_refresh.png){style="width: 1000px; border-radius: 15px; border: 5px solid #333333"}</center>
...
microbiome::taxa(pseq_relabund)
microbiome::taxa(phylum_rareaggregate_pseq)
#Taxa groups lost via aggregation
setdiff(microbiome::taxa(pseq_relabund), microbiome::taxa(phylum_rareaggregate_pseq))
```
...
for(i in 1:ncol(cbn)){
  ...
  #Ensure to change metadata group if using different grouping (site)
  wunifrac_pairwise_adonis <- vegan::adonis2(wunifrac_dist_mat ~ site, 
                                             data = metadf_subset, by = "margin")
...
for(i in 1:ncol(cbn)){
  ...
  #Ensure to change metadata group if using different grouping (site & media)
  wunifrac_pairwise_adonis <- vegan::adonis2(wunifrac_dist_mat ~ site.media, 
                                             data = metadf_subset, by = "margin")
  #Add the gorup names and p-value to the main data frame
  pairwise_permanova_df[i,1:2] <- cbn[,i]
  pairwise_permanova_df[i,3] <- wunifrac_pairwise_adonis[1,"Pr(>F)"]
...
<center>![](figures/mamba_logo.png){style="border-radius: 15px; width: 20%; border: 5px solid #333333"}</center>
