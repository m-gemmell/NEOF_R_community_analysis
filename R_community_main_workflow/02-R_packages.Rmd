...
There are lot of R packages/libraries. These can be downloaded with an R function. The most common function used is `install.packages()`. Once you have installed an R package you won't need to install it again on your computer for your current version of R. Unfortunately, if you install a new version of R you will need to reinstall all your packages. I generally install each package when I need them rather than doing them all, once I have a new version of R.

An R package is not ready to use when you first install it. Every time you open up a new R environment, such as opening up RStudio, you will need to load the package. This can be carried out with the function `library()`. I generally have a list of `library()` functions at the top of my R scripts (`.R` files) for all the packages I use in the script.

Throughout this course you will get a lot of practice of installing and loading various packages.
...
An R library is then the directory containing the R package.

Because of this, the terms R package and R library may be used synonymously. We will use the term package in this workshop.
`r unhide()`

As we will be using a lot of packages we shall use a double colons to specify which package each function belongs to, unless the function is from base R. For example if we the function `summarize_phyloseq()` from the package `microbiome` we would type it like below:
...
An example of `ggplot2` code and its visualisation output is:
